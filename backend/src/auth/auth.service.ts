...
export class AuthService {
  ...
  findRefreshToken = async (id: string): Promise<RefreshToken | any> => {
    return await this.refreshTokenService.findRefreshToken(id); 
   };

  removeRefreshToken = async (id: string): Promise<RefreshToken> => {
    return await this.refreshTokenService.removeRefreshToken(id);
  };

  removeManyRefreshToken = async (query: any): Promise<RefreshToken[]> => {
    return await this.refreshTokenService.removeManyRefreshToken(query);
  };

  resetPassword = async({email,otp,token, confirmPassword, newPassword}:ResetPasswordDto): Promise<any> =>{
  ...
  if (storedOtp === otp && storedToken === token) {
    ...
    await this.userService.updateUser(this.requestService.getUser().id,{password:hashedPassword})
  ...
  verifyUserEmail = async({email,otp,token}:VerifyUserEmailDto): Promise<any> =>{
  ...
  if (storedOtp === otp && storedToken === token) {
    // Update password in the database
    await this.userService.updateUser(this.requestService.getUser().id,{verificationStatus:true})
    // Delete OTP and token from Redis
    await this.redisService.delete(`otp:${email}`);
    await this.redisService.delete(`token:${email}`);
    return { message: 'Password reset successful' };
  ...
  resetPasswordRequest = async({email}:ResetPasswordRequestDto): Promise<any> =>{
    const foundUser = this.userService.findUser({where:{email}});
    ...
    await this.redisService.set(`otp:${email}`, otp, ttl);
    await this.redisService.set(`token:${email}`, token, ttl);
    this.eventEmitter.emit('reset-password-requested', {otp,token,foundUser});
