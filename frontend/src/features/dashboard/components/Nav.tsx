...
const Nav = ({ pageData }) => {
  ...
  const handleThemeToggle = () => {
    ...
    document.body.setAttribute('data-theme-version', themeMode ? 'light' : 'dark');
    setTheme(themeMode ? 'light' : 'dark');
  };

  const handleMenuTabs = () => {
    if (width as number <= 991) {
      // Example of toggle functionality without jQuery
      const toggleMenu = (isOpen) => {
        const contentBox = document.querySelector('.fixed-content-box')!;
      ...
      document.querySelectorAll('.menu-tabs .nav-link').forEach((link) => {
        link.addEventListener('click', () => {
          toggleMenu(link.classList.toggle('open'));
        });
      });

      document?.querySelector('.close-fixed-content')?.addEventListener('click', () => {
        toggleMenu(false);
      });
    }
  };
  ...
  const handleAllChecked = () => {
    ...
    const checkboxes = document.querySelectorAll('td input[type="checkbox"], .email-list .custom-checkbox input[type="checkbox"]')!;
    ...
    <nav className="navbar navbar-expand">
      <div className="collapse navbar-collapse justify-content-between">
        ...
        <ul className="navbar-nav header-right main-notification">
          ...
          <li className="nav-item dropdown notification_dropdown">
            <Link className="nav-link bell dz-theme-mode" to="" onClick={handleThemeToggle}>
          ...
          <li className="nav-item dropdown notification_dropdown">
            <Link className="nav-link bell dz-fullscreen" to="" onClick={toggleFullscreen}>
              <svg id="icon-full" viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round">
                <path d="M8 3H5a2 2 0 0 0-2 2v3m18 0V5a2 2 0 0 0-2-2h-3m0 18h3a2 2 0 0 0 2-2v-3M3 16v3a2 2 0 0 0 2 2h3"></path>
              </svg>
              <svg id="icon-minimize" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M8 3v3a2 2 0 0 1-2 2H3m18 0h-3a2 2 0 0 1-2-2V3m0 18v-3a2 2 0 0 1 2-2h3M3 16h3a2 2 0 0 1 2 2v3"></path>
              </svg>
            </Link>
          </li>
          <li className="nav-item dropdown notification_dropdown">
            <Link className={`nav-link ai-icon ${toggleNotification ? 'show' : ''}`} to="" onClick={() => setToggleNotification(!toggleNotification)}>
            <svg className="bell-icon" width="24" height="24" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
            ...
            <div className={`dropdown-menu dropdown-menu-end ${toggleNotification ? 'show' : ''}`}>
              <Notification />
            </div>
          </li>
          <li className="nav-item dropdown notification_dropdown">
                                <div className="nav-link bell bell-link" onClick={()=>setShowSideChat(prev => !prev)}>
                                    <svg width="24" height="24" viewBox="0 0 28 28" fill="currentColor" xmlns="../../www.w3.org/2000/svg.html">
