...
import { useLogoutMutation, useSendLogoutMutation } from '@auth/slices/authApi.slice';
...
import { appRoutes, generatePath } from '@app/app/routes';
...
const Nav = ({ pageData }) => {
  ...
  const { settings: { companyDetails: { siteName } = {} } = {} } =
    useSelector(useCompanyInfo);
  const { settings: { siteImages: { logo, logoDark } = {} } = {} } =
...
const toggleFullscreen = () => {
  ...
  const handleMenuTabs = () => {
    if ((width as number) <= 991) {
      ...
      document
        ?.querySelector(".close-fixed-content")
  ...
  const handleAllChecked = () => {
    ...
    const checkboxes = document.querySelectorAll(
      'td input[type="checkbox"], .email-list .custom-checkbox input[type="checkbox"]'
    )!;
  ...
  //redux-rtkquery
  const [logOutUser, { isLoading: isLogoutLoading, isSuccess:isLogoutSuccess, isError: isLogoutError, error:logoutError }] =
    useLogoutMutation();
  const handleLogout = async () => {
    await logOutUser();
    navigate(generatePath(appRoutes["login"]) as string);
  };
    ...
    <nav className="navbar navbar-expand">
      <div className="collapse navbar-collapse justify-content-between">
        ...
        <ul className="navbar-nav header-right main-notification">
          ...
          <li className="nav-item dropdown notification_dropdown">
            <Link
              className={`nav-link ai-icon ${toggleNotification ? "show" : ""}`}
              to=""
          ...
          <li className="nav-item dropdown notification_dropdown">
            <div
              ...
              onClick={() => setShowSideChat((prev) => !prev)}
            >
              <svg
                width="24"
                height="24"
          ...
          <li className="nav-item dropdown notification_dropdown d-sm-flex d-none">
            <Link
              className={
                toggleAlert ? "show nav-link ai-icon" : "nav-link  ai-icon"
              }
              to=""
              role="button"
