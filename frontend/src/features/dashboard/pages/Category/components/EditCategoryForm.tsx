...
import { useUpdateCategoryMutation } from '../slices/categoryApi.slice';
import handleApiErrors from '@utils/handleApiErrors';
import tinyMCEInit from '@configs/tinymce.config';
import { Category } from '@props/categoryProps';
...
const EditCategoryModal = ({ modalData: { data, showModal } }: ModalDataProps<HomeCategory>) => {
  ...
  const updates: SubmitHandler<FieldValues> = useCallback(async (formFields, e) => {
    e?.preventDefault();
          await updateCategory({id:data.id, formFields });
  },
   [data, updateCategory]);
  ...
  return (
    ...
    <ModalComponent {...{size:((width as number) < 600)? "xl": "lg", header:{show:true,title:'Edit Category'},modalStates:{show,handleOpen,handleClose}}} >
      <form onSubmit={handleSubmit(updates)} encType="multipart/form-data">
          <div className="row">
          ...
          <Button variant="secondary" size="sm" type="submit" disabled={isSubmitting}>
            {isSubmitting ? <PulseLoader loading={isSubmitting} color="#ffffff" size="0.7rem" /> : 'Update Category'}
          </Button>
          </div>
      </form>
    </ModalComponent>      
