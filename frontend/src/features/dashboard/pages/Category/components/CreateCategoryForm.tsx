...
import { useAddNewCategoryMutation, useGetCategoriesQuery } from '../slices/categoryApi.slice';
...
const CreateCategoryForm = () => {
  ...
  const [addNewCategory, {  isSuccess, isError, error }] = useAddNewCategoryMutation();
    const {
            data:categories,
            isLoading,
  ...
  const { register, control, handleSubmit, clearErrors, setError, formState: { errors, isSubmitting }, setValue, reset, watch } = useForm<FormInputs>({
    defaultValues: {
      name: '',
      parentId: '',
      for: '',
  ...
  return (
    <>
      <button type="button" className="btn btn-primary btn-sm mb-2" onClick={handleOpen}>Add New</button>
          ...
          <ModalComponent {...{size:((width as number) < 600)? "xl": "lg", header:{show:true,title:'Add New Category'},modalStates:{show,handleOpen,handleClose}}} >
      ...
      {/* Icon type */}
                  <div className="mb-3 col-md-3">
                    <label className="form-label"><strong>For Entity</strong></label>
                    <select
                      id="for"
                      className={`form-control default-select form-control wide ${errors.for ? 'is-invalid' : ''}`}
                 ...
                 {categories && (
                  ...
                  <div className="my-20 col-md-12 d-flex justify-content-between">
                    ...
                    <label className='p-10' htmlFor="addIcon">
                      {addIcon ? <BsToggleOn className='text-primary' fontSize={'2rem'} /> : <BsToggleOff className='text-default' fontSize={'2rem'} />}
                    </label>
                    <input
                      type="checkbox"
                      id="addIcon"
                  ...
                  <div className="mb-3 col-md-4">
                    <label className="form-label"><strong>Icon Type</strong></label>
                    <select
                      id="iconType"
                      className={`form-control default-select form-control wide ${errors.iconType ? 'is-invalid' : ''}`}
                      {...register('iconType')}
                    >
                      <option selected value={CategoryIconType.ICON}>Icon Class</option>
                      <option value={CategoryIconType.SVG}>SVG</option>
                      <option value={CategoryIconType.IMAGE}>Image upload</option>
                    </select>
                    <div className="invalid-feedback">{errors.iconType?.message}</div>
                  </div>
                      {
                      ...
                      }
                      {
                          iconType === CategoryIconType.SVG && (
                              <div className="col-12">
                                  <label className="form-label"><strong>SVG Icon</strong></label>
                                  <Editor
                                    tinymceScriptSrc="/tinymce/tinymce.min.js"
                                    onEditorChange={(newValue) => setValue('icon', newValue)}
                                    value={watch('icon')}
                                    init={tinyMCEInit}
                                  />
                                </div>
