...
const Category = ({ pageData }: PageProps) => {
  const [modalData, setModalData] = useState<ModalDataProps<CategoryProps>["modalData"]>({
    data: null,
    showModal: false,
  });
  ...
  const showEditForm = (modalData: ModalDataProps<CategoryProps>["modalData"]) => {
    setModalData(modalData);
  };

  const tableHead = ["S/N", "Icon", "Name", "Parent", "For Entity", "Status", "Action"];
  ...
  const slots = {
    1: (data: any, row: any) => (
     row.icon? <LightGallery plugins={[lgZoom]}>
        <a href={`${CATEGORY_ASSETS}${row.icon}`} data-lightbox={`icon-${row.id}`} data-exthumbicon={CATEGORY_ASSETS + row.icon} data-src={CATEGORY_ASSETS + row.icon} data-name={row.name}>
          <img src={`${CATEGORY_ASSETS}${row.icon}`} alt={row.name} width="150" />
        </a>
      </LightGallery>: <p></p>
    ),
    // 4: (data: any, row: any) => (
    ...
    6: (data: any, row: any) => (
      <div className="d-flex">
        <button type="button" className="btn btn-info light shadow btn-xs sharp me-1" onClick={() => showEditForm({ data: row.category, showModal: true })}>
          <i className="fas fa-pencil-alt"></i>
        </button>
        <button type="button" className="btn btn-danger light shadow btn-xs sharp" onClick={() => onDeleteCategory(row.category.id)}>
          ...
          const categoriesTableData =  categories?.entities && Object.values(categories?.entities)?.map((category: CategoryProps, i: number) =>( {
            i:(i + 1),
            icon:category.icon,
            name:category?.name,
            ...
            createdAt:  new Date(category?.createdAt).toLocaleString('en-US', { day: 'numeric', month: 'long', year:'numeric' }),
            category:category
        })) || [];
        ;
