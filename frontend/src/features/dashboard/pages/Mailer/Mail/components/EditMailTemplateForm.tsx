...
import { useUpdateMailTemplateMutation } from '../../slices/mailerApi.slice';
import showToast from '@utils/showToast';
import handleApiErrors from '@utils/handleApiErrors';
import tinyMCEInit from '@configs/tinymce.config';
import {MailTemplateProps} from '@props/MailerProps';
...
interface ModalDataProps {
  modalData: {
    data: MailTemplateProps | null;
    showModal: boolean;
  };
}

const EditMailTemplateModal = ({ modalData: { data, showModal } }: ModalDataProps) => {
  const [previewImage, setPreviewImage] = useState<string>(`${SERVICE_ASSETS}${data?.image}`);
  const [updateMailTemplate, { isLoading, isSuccess, isError, error }] = useUpdateMailTemplateMutation();
  ...
  const onSubmit: SubmitHandler<FieldValues> = async (formFields, e) => {
    ...
    await updateMailTemplate(formData);
  };
...
export default React.memo(EditMailTemplateModal);
