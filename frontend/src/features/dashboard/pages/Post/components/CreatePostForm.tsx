  ...
  return (
    <>
        ...
        Add New
          ...
          <ModalComponent {...{size:((width as  number)< 600)? "xl": "lg", header:{show:true,title:'Add New Post'},modalStates:{show,handleOpen,handleClose}}} >
        ...
        <form onKeyDown={checkKeyDown} onSubmit={(e) => handleSubmit(onSubmit)(e) } encType="multipart/form-data">
          ...
          <Button variant="secondary" size="sm" type="submit" disabled={isSubmitting}>
            {isSubmitting ? <PulseLoader loading={isSubmitting} color="#ffffff" size="0.7rem" /> : `${capitalizeFirstLettergetValues(('status'))} Post`}
          </Button>
          </div>
        </form>
      </ModalComponent>
